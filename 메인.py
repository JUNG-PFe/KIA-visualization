...
if st.session_state.logged_in:
    ...
    st.markdown(
        """
        <h1 style="text-align: center; margin-top: 10px; font-size: 4em;">KIA 데이터 분석</h1>
        """,
    ...
    st.write("---")
    st.write("타자")
    st.write("1.타자 호크아이 분석 23-24 : 기본 분석값, 타구 Spray chart, 컨택 포인트, 타구 속도와 각도 확인")
    st.write("2.타자 호크아이 선수 비교 타격 : 선수 간, 기간 간 Spray chart 컨택 포인트 비교")
    st.write("3.타자 호크아이 존별 데이터분석 : 스트라이크 존 안 존별, 볼 구역 포함 존별 결과 비율 확인")
    st.write("4.타자 PTS 데이터 타격 : 존별 결과(e.g. 인플레이%, 헛스윙% 등) , 결과 별 타구 속도 및 각도 확인")
    st.write("---")
    st.write("투수")
    st.write("5.호크아이 데이터 23-24 : 기본 트래킹 데이터 확인, 구종별 로케이션, 무브먼트 차트 확인")
    st.write("6.호크아이 선수비교 : 선수간 트래킹 데이터 비교")
    st.write("7.호크아이 피칭궤적 : 피칭궤적")
    st.write("8.호크아이 트렌드 분석 : 각 선수 트래킹 데이터 추이 분석")
    st.write("9.PTS 데이터 24 : 24시즌 구속, 스크라이트, 볼 비율 등과 투구 로케이션 확인")
    st.write("10.PTS 데이터 존별 타구속도 24 : 24시즌 투수 존별 타구 속도 확인")
